	...
	var App={
		...
		initUI:function(){
			if(location.search.indexOf('estateid')>=0){
				...
				var estatename=decodeURIComponent(location.search.substring(location.search.indexOf('estatename=')+11));
				$('.selected-wrap span').text(estatename);
				this.hasEstate=true;
			...
			$('.J_Recommend').on('click',function(){
				...
				self.checkBaobei(cid,function(data){
					if(data){
						...
						if(confirm('确定对'+$('.J_List .checked .name').text()+'推荐'+self.estatename+'楼盘？')){
							self.addBaobei(cid);
